...
We will measure the effect of single imputation and multiple imputation using the PTRATIO. Mice uses conditional multiple imputation that does not require a normal distribution. For our single imputation method we will use a global mean to replace our missing values for PTRATIO.

We impute the data with Mice with default parameters and return 5 different imputations that are then pooled together. The default imputation method is applied or predictive mean matching. (add additional notes describing predictive mean matching)
...
We incorporate all 5 imputation possibilities back into our data and transform it into a long dataframe. Each of this imputed values represent a sample from a distribution. 
