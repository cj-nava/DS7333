...
We observe 9348 emails in this classification problem to identify spam given 29 predictor variables.

The predictor variables characterize our email header and body and include factor level data such as isDear which tells us if the word dear is in the email body and also other non-categorical variables like numLines which is discrete values for the number of lines in each of our email examples. We search for Spam in a body of emails containing ham, which is not spam.

Note spam emails are identified with the value T and ham as F. Our response is binary which warrants a categorical approach with CART using Rpart. A definition summary of the predictor variables is found below.
...
The data requires some preprocessing as several fields contain no values and there is an imbalance in our class of emails with 1918 spam and 5561 ham examples. Before addressing the data quality issues we split our data into a training and test sets as the imputation function finds the replacement values based on our training set only. 

We split our data into an 80/20 split consisting of a training and test set not as a sampling method but in order to model our training data with cross validation and assume the test set is a brand new data set we want to test with for accuracy. 
...
Several variables have missing values, particularly numRec, or the number of receipients in the email. This justifies imputation for our data.
...
We double check our test and training data for missing values.Voila! An imputed data set.
...
The number of spam observations is significanly less than our ham emails. We have 1918 spam against 5561 ham emails.
We can approach this by decreasing the number of ham emails or increasing the number of spam emails to match the volume of ham.
To prevent a reduction in the number of observations we apply the upsample method. This method will increase our spam volume to match our ham volume, resulting in  5561 spam and ham emails.
...
The upsample function from Caret increases the number of Spam observations to match our ham emails. The end result is 5561 observations for ham and spam emails.
...
These splits are determined by measuring the purity of the nodes, which looks at the probability of the classes using the formula below we will compute the Ginni. The leaf nodes determine our end result and are not determined by the mean of the node but purity which is more synonymous of the mode of the leaf node. Nodes closer to 0 represent more homogenous nodes and those close to .5 least impure. For our specific example we are trying to identify Spam so we look at probability of selecting spam. 
...
Fitting our training data without balancing our classeses results in a different tree in terms of number of nodes and depth. 
...
We measure how each of these models perform by looking at confusion matrices that can be dissected to evaluate different aspects of our model; True Positive, False Positive, True Negative, and False Negative. The confusion matrix shows all these values and helps us compute the overal accuracy by adding all the values we correctly guessed over the total given by the formula below. We can apply this to both our training and test sets. Accuracy is higher for our training and test samples without adjusting for class imbalance. This is the result of an increase in type 2 errors as we are allowing more mistakes where we inaccurately over identify spam.
...
We will compute additional performance metrics using our results from our test set and unbalanced data set which performed better.
These metrics use our confusion matrix data to look at the accuracy for calculating true positives and true negatives. The higher the value the better our performance.
...
WApplying the new cp and maxdepth increases our accuracy to 94% when applied to our test set. The resulting tree however is more complex as our cp threshold is lower allowing for a more complex tree as seen below.
...
The trade-off of a tuned model is an increased accuracy, however this results in a very cumbersome tree with many leaf nodes and depth. Translating the tree to a non-technical audience would be close to impossible without putting them to sleep. However our accuracy would shine as our business partners may only be interested in how well our model performs and not the guts of our tree. 